...
{
    public class BlogApiTests : IClassFixture<DataTestFixture>
    {
        private readonly DataTestFixture _fixture;

        public BlogApiTests(DataTestFixture fixture)
        {
            _fixture = fixture;
        }
        ...
        {
            var cat = await _fixture.Api.SaveCategoryAsync(new Category() { Id = Guid.NewGuid().ToString(), Name = $"Category {DateTime.Now}" });
            var tag = await _fixture.Api.SaveTagAsync(new Tag() { Id = Guid.NewGuid().ToString(), Name = $"Tag {DateTime.Now}" });
            var post = await _fixture.Api.SaveBlogPostAsync(new BlogPost() { Id = Guid.NewGuid().ToString(), Title = $"Title {DateTime.Now}", Category = cat, Tags = new List<Tag> { tag! }, PublishDate = DateTime.Now, Text = "Text" });
